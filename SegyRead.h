...
/* ShotStruct containing single-shot parameters and data */
typedef struct ShotStruct{
        int is;		/* shot index in multi shot data */
        int ntr;	/* trace number in the single-shot */
        int nt;		/* number of time samples */
	int irug; 	/* index of rugged topography */
        float dt;	/* time interval of trace samples */
        float ft;	/* first sample's time in traces  */
        float xs;
        float ys;
	float zs;	/* source point coordinate */
	float offset;	/* maximum offset */
	float zr;	/* depth of receiver point (valid when irug=0) */
        float *coordx;
        float *coordy;
        float *coordz;	/* array[ntr] containing receiver coordinates */
        float **data;	/* array[ntr][nt] containing seismic data */

}Shot;


/* ModelStruct containing model parameters and data */
typedef struct ModelStruct{
        int nx;		/* x-dimension of model */
        int ny;		/* y-dimension of model */
        int nz;		/* z-dimension of model */
	int wtype;	/* wave type index set as middle variable */
        float dx;
        float dy;
        float dz;	/* spacing of model grid */
        float fx;
        float fy;
        float fz;	/* first sample of model */
		float vpavg;	/* average P velocity */
		float vpmin;	/* minimum P velocity */
        float **vp;	/* array[ny][nx][nz] of P velocity */
	float **vel;		/* middle variable */
} Model;
...
typedef struct coordStruct{
        float x;
        float y;
	float z; /* 3d coordinate */
}coord;

/* broadcast of parameters */
typedef struct parBcastStruct{	/* param need to be broadcasted */
        int nx; int ny; int nz; /* dimension of models */
        float dx; float dy; float dz;	/* spacing of model grid */
        float fx; float fy; float fz;	/* first sample of model */
        float vpavg; float vpmin;  /* vel value */
        int nt; float dt; int nstotal; int ntrps_max; /* data param */
}parBcast;

/* send and receive paramaters of single shot */
typedef struct parSendRecvStruct{ /* param need to be send to diff processes */
        int is;  /* index of shot */
        int ntr; /* number of traces in each shot gather */
        float xs;
        float ys;
        float zs;/* source coordinate */
}parSendRecv;
...
void getShotInfo_su(FILE *data, int *nt, float *dt, long *ntrtotal, int *nstotal, int *ntrps_max,float *scalel,float *scalco,int endian);
